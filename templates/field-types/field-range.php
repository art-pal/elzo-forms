...
<div class="elzo-forms-range-slider-wrapper elzo-forms-range-type-<?php echo esc_attr($range_type); ?>">
    ...
    <div class="elzo-forms-range-slider">
        ...
        <?php if($range_type == 'range_2'){ ?>
            <input type="range" class="elzo-forms-range-slider-min" value="<?php echo esc_attr($value_min); ?>" step="<?php echo esc_attr($step); ?>" min="<?php echo esc_attr($min_value); ?>" max="<?php echo esc_attr($max_value); ?>">
        <?php } ?>
        <input type="range" <?php echo $range_type != 'range_2' ? 'id="' . esc_attr($id) . '" name="' . esc_attr($name) . '"' : ''; ?> class="elzo-forms-range-slider-max" value="<?php echo esc_attr($value_max); ?>" step="<?php echo esc_attr($step); ?>" min="<?php echo esc_attr($min_value); ?>" max="<?php echo esc_attr($max_value); ?>">
    </div>
        ...
        <div class="elzo-forms-range-slider-footer elzo-forms-row elzo-forms-row-auto-cols elzo-forms-justify-content-between">
            ...
            <div class="elzo-forms-column">
                <?php if($input_prepend || $input_append){ ?>
                    <div class="elzo-forms-field-input-group">
                    <?php if($input_prepend){ ?>
                        <label for="<?php echo esc_attr($id); ?>-max-input" class="elzo-forms-field-prepend"><?php echo esc_html($input_prepend); ?></label>
                    <?php } ?>
                <?php } ?>
                <input type="number" id="<?php echo esc_attr($id); ?>-max-input" class="elzo-forms-field-control elzo-forms-range-slider-max-input<?php echo $input_prepend || $input_append ? ' elzo-forms-field-input-group-input' : ''; ?>" value="<?php echo esc_attr($value_max); ?>" placeholder="<?php echo esc_attr($value_max); ?>" step="<?php echo esc_attr($step); ?>" min="<?php echo esc_attr($min_value); ?>" max="<?php echo esc_attr($max_value); ?>">
                <?php if($input_prepend || $input_append){ ?>
                    <div class="elzo-forms-input-group-pseudo-border"></div>
                    <?php if($input_append){ ?>
                        <label for="<?php echo esc_attr($id); ?>-max-input" class="elzo-forms-field-append"><?php echo esc_html($input_append); ?></label>
                    <?php } ?>
                    </div>
                <?php } ?>
            </div>
    ...
    <?php } ?>
    <?php if($range_type == 'range_2'){ ?>
        <input type="hidden" name="<?php echo esc_attr($name); ?>" class="<?php echo esc_attr($class); ?> elzo-forms-range-slider-real-input" id="<?php echo esc_attr($id); ?>" <?php echo $required && !$logic_rules ? 'required' : ''; ?> value="<?php echo esc_attr($range_type == 'range_2' ? $value_min . ' - ' . $value_max : $value_max); ?>" step="<?php echo esc_attr($step); ?>" min="<?php echo esc_attr($min_value); ?>" max="<?php echo esc_attr($max_value); ?>">
    <?php } ?>
</div>
